...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def p_sample(self, x, t, context, clip_denoised, sample_mode="ddpm", eta=0):#函数 p_sample 是在分布P(Xt-1|Xt)中采样Xt-1
    ...
    def compress(
        ...
        with torch.no_grad():  # 确保不计算梯度
            context_dict = self.context_fn(input_image, ref_image)  # 这里范围是（0-1）
        context = context_dict["output"]
        if isinstance(context, (list, tuple)):
            for i in range(len(context)):
                context[i] = context[i] * 2.0 - 1.0
        else:
            raise TypeError("Expected context to be a list or tuple")
        ...
        return (
            self.p_sample_loop(
                input_image.shape, context, sample_mode, init=init, eta=eta
            ),
            context_dict["bppb"].mean() if bpp_return_mean else context
        )
    ...
    def p_losses(self, x_input, output_dict, t):
        x_start = x_input * 2.0 - 1.0
        noise = torch.randn_like(x_start)

        context = output_dict["output"]
        if isinstance(context, (list, tuple)):
            for i in range(len(context)):
                context[i] = context[i] * 2.0 - 1.0
        else:
        ...
        fx = self.denoise_fn(
            x_noisy, t.float().unsqueeze(-1) / self.num_timesteps, context=context
        )
        # x_recon = self.denoise_fn(
        #     x_noisy, self.train_sqrt_alphas_cumprod[t], context=context_dict["output"]
        # )

        print("Pred mode:", self.pred_mode)
        if self.pred_mode == "noise":
        ...
        if self.aux_loss_weight > 0:
            ...
            print("Aux loss type:", self.aux_loss_type)
            ...
            loss = self.lagrangian_beta * output_dict["bppb"].mean() + err
        # loss = err.mean()

        return output_dict["bppb"],pred_x0,loss
    ...
    def forward(self, input_image, ref_image):
        ...
        if self.vbr:
            ...
            bitrate_scale = None
            print("none bitrate_scale")

        with torch.no_grad():  # 确保不计算梯度
            output_dict = self.context_fn(input_image, ref_image)  # 这里范围是（0-1）

        bppb, x_output, loss = self.p_losses(input_image, output_dict, t)
        return bppb, x_output, loss
