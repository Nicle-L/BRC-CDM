...
class ScaleSpaceFlow(nn.Module):
    ...
    def warp_volume(self, volume, flow, scale_field, padding_mode: str = "border"):
        ...
        volume_grid = torch.cat((update_grid, update_scale), dim=-1).unsqueeze(
            1)  # 在最后一个维度进行拼接得到，（B,H,W,C+2），最后增加一个维度，（B,1,H,W,C+2）
...
def main():
        ...
        compressor = ScaleSpaceFlow()
        compressor.to(device)
        num_pixels = 256*256
        # 模拟输入数据，假设输入图像为 input_image，参考帧为 referframe
        input_image = torch.randn(1, 3, 256, 256)  # 假设输入图像尺寸为 256x256，3通道
        ref_image = torch.randn(1, 3, 256, 256)  # 假设参考帧尺寸与输入图像相同
        criterion_mse = nn.MSELoss()
        # 执行前向传播
        input_image, ref_image = input_image.cuda(),ref_image.cuda()
        output_dict = compressor(input_image, ref_image)
        bppb = output_dict["bppb"]
        x_rec_list = output_dict["output"]
        x_rec = x_rec_list[0]

        # 打印输出结果
        #print("Clipped reconstructed image shape:", x_rec.shape)
        #print("Mean squared error loss:", distortion.item())
        print("bpp loss:", bppb.item())
        #print("bpp_res loss:", res_likelihoods_sum.item())
